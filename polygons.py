...
def drawPolygons(polygons, color):
    printMatrix(polygons)
    tri = 0
    i = 0
    while  i < len(polygons):
        tri +=1
        if dotProduct(crossProduct(surfaceVector(polygons[i], polygons[i + 1]),
                                   surfaceVector(polygons[i], polygons[i + 2])),
                      viewVector()) > 0:
            drawLine([int(polygons[i][0]), int(polygons[i][1])],
                     [int(polygons[i+1][0]), int(polygons[i+1][1])], color)
            i +=1
            drawLine([int(polygons[i][0]), int(polygons[i][1])],
                     [int(polygons[i+1][0]), int(polygons[i+1][1])], color)
            i+=1
            drawLine([int(polygons[i][0]), int(polygons[i][1])],
                     [int(polygons[i-2][0]), int(polygons[i-2][1])], color)
            i+=1
        else :
            i += 3
    print("number of triangles: {}".format(tri))
